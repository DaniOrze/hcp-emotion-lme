...
def figure1_methods_overview():
    ...
    lme_icc_path = LME_VOXELWISE / "lme_icc.nii.gz"
    ...
    if glm_zmap.exists() and lme_zmap.exists():
        ...
        vmax = max(
            np.nanpercentile(np.abs(glm_data[glm_data != 0]), 99.5),
            np.nanpercentile(np.abs(lme_data[lme_data != 0]), 99.5),
        )
    ...
    if lme_icc_path.exists():
        plotting.plot_stat_map(
            str(lme_icc_path),
            threshold=0.0,
            display_mode='z',
            cut_coords=[-25, -10, 0, 10, 25, 45],
            title='E) Intraclass Correlation Coefficient (ICC) Map — LME',
            axes=ax5,
            colorbar=True,
            annotate=True,
            cmap='hot',
        ...
        ax5.text(0.5, 0.5,
                'ICC map not found.\n'
                'Run main_lme_voxelwise_v2.py to generate lme_icc.nii.gz',
                ha='center', va='center', transform=ax5.transAxes)
        ax5.set_title('E) ICC Map')
    
    plt.savefig(FIGURES / 'figure1_brain_maps.png')
    plt.savefig(FIGURES / 'figure1_brain_maps.pdf')
    plt.close()
    logger.info("Saved Figure 1: Brain maps (with ICC map — Major Concern 4)")
...
def figure2_roi_comparison():
    ...
    diff = abs(glm_df.loc[common, 'cohens_d'] - lme_df2.loc[common, 'cohens_d'])
    top_rois = diff.nlargest(10).index  # só os 10 mais relevantes

    texts = []
    for roi in common:
        if roi in top_rois:
            texts.append(ax.text(
                glm_df.loc[roi, 'cohens_d'],
                lme_df2.loc[roi, 'cohens_d'],
                roi,
                fontsize=6,
            ))
...
def figure4_computational():
    ...
    ax = axes[2]
    lme_data = df[(df['method'] == 'LME') & (df['convergence_rate'].notna())]
    if not lme_data.empty:
        grouped = lme_data.groupby('n_subjects')['convergence_rate'].agg(['mean', 'std'])
        if len(grouped) > 0:
            ax.errorbar(grouped.index, grouped['mean'] * 100, 
                       yerr=grouped['std'] * 100 if 'std' in grouped else None,
                       marker='o', capsize=3, color=COLORS['LME'])
            ax.set_xlabel('Number of Subjects')
            ax.set_ylabel('Convergence Rate (%)')
            ax.set_title('C) LME Convergence vs Sample Size')
            ax.set_ylim(0, 105)
            ax.axhline(100, color='green', linestyle='--', alpha=0.3, label='100%')
            ax.axhline(95, color='orange', linestyle='--', alpha=0.3, label='95%')
            ax.legend()
    else:
        conv_data = df[(df['method'] == 'LME') & (df['n_voxels'] == 1000)]
        if not conv_data.empty and 'converged' in conv_data.columns:
            grouped = conv_data.groupby('n_subjects')['converged'].mean()
            ax.plot(grouped.index, grouped.values * 100, 
                   marker='o', color=COLORS['LME'])
            ax.set_xlabel('Number of Subjects')
            ax.set_ylabel('Convergence Rate (%)')
            ax.set_title('C) LME Convergence vs Sample Size')
            ax.set_ylim(0, 105)
    ...
    plt.close()
    logger.info("Saved Figure 4: Computational benchmark (3 panels)")
