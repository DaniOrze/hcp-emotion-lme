...
def create_benchmark_figure(results_df, output_dir):
    ...
    ax = axes[1, 0]
    n_voxels_brain = 150000
    n_subjects_typical = 143
    ...
    NOTE (mn2 — Time discrepancy):
    The projected time ({lme_total_hours:.1f}h for {n_voxels_brain:,}
    grayordinates) underestimates the observed whole-
    brain time (~12.4h for 356,749 grayordinates) by a
    factor of ~6.7x. This discrepancy is explained by:
    (a) chunk processing overhead (I/O, memory
        allocation/deallocation per chunk),
    (b) periodic checkpointing to disk,
    (c) OS-level memory pressure and GC overhead
        with large DataFrames, and
    (d) the projection uses average per-voxel time
        measured on small batches, which does not
        capture the non-linear scaling of memory
        management with increasing working set size.
