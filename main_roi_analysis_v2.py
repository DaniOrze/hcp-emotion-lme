...
def run_roi_glm(df, roi_name):
    ...
    between_var = np.var(subject_means, ddof=1)
    
    cohens_d = float(np.mean(subject_means) / np.sqrt(total_var)) if total_var > 0 else 0.0
    
    cohens_d_conventional = float(np.mean(subject_means) / np.sqrt(between_var)) if between_var > 0 else 0.0
    ...
    return {
        ...
        'cohens_d': cohens_d,
        'cohens_d_conventional': cohens_d_conventional,  # mn1
        'converged': True
    }
...
def run_roi_lme_ultra_robust(df, roi_name, max_attempts=5):
    ...
    res = fit_lme_single_voxel(
        betas, run_info,
        include_run_effect=use_run_effect,
        reml=True,
    )
    
    if not res.get('converged', False):
        logger.warning(f"LME failed for {roi_name} (backend: {res.get('backend', 'unknown')})")
        return None
    
    mean_beta = res['fixed_effect']
    se = res['std_err']
    t_val = res.get('t_value', res.get('z_score', np.nan))
    p_val = res['p_value']
    icc = res.get('icc', np.nan)
    var_between = res.get('var_between', np.nan)
    var_within = res.get('var_within', np.nan)
    
    total_variance = var_between + var_within if (np.isfinite(var_between) and np.isfinite(var_within)) else np.nan
    
    cohens_d = mean_beta / np.sqrt(total_variance) if (np.isfinite(total_variance) and total_variance > 0) else np.nan
    
    cohens_d_conventional = mean_beta / np.sqrt(var_between) if (np.isfinite(var_between) and var_between > 0) else np.nan
    ...
    result = {
        ...
        'cohens_d_conventional': float(cohens_d_conventional) if np.isfinite(cohens_d_conventional) else np.nan,  # mn1
        'var_subject': float(var_between) if np.isfinite(var_between) else np.nan,
        'var_residual': float(var_within) if np.isfinite(var_within) else np.nan,
    ...
    logger.debug(f"LME succeeded for {roi_name} (backend: {result['backend']})")
    return result
...
def main():
    logger.info("="*50)
    logger.info("ROI ANALYSIS: GLM vs LME")
