...
def load_all_runs_betas(subject_ids, contrast_name="fear_minus_neut",
        ...
        for run in runs:
            ...
            try:
                ...
                if smooth_fwhm is not None and smooth_fwhm > 0:
                    img = smooth_img(img, fwhm=smooth_fwhm)
    ...
    if smooth_fwhm is not None and smooth_fwhm > 0:
        logger.info(f"Applied {smooth_fwhm} mm FWHM smoothing to all beta maps")
    else:
        logger.info("No smoothing applied (smooth_fwhm=None or 0)")
    ...
    for chunk_idx in tqdm(range(n_chunks), desc="LME chunks"):
        ...
        def process_subchunk(job_idx):
            ...
            return fit_lme_chunk(
                ...
                include_run_effect=include_run_effect
            )
...
def create_result_maps(results_df, mask_img, output_dir, output_prefix="lme"):
    ...
    nib.save(nib.Nifti1Image(z_map, affine), output_dir / f"{output_prefix}_zmap.nii.gz")
    nib.save(nib.Nifti1Image(p_map, affine), output_dir / f"{output_prefix}_pmap.nii.gz")
    nib.save(nib.Nifti1Image(beta_map, affine), output_dir / f"{output_prefix}_beta.nii.gz")
    nib.save(nib.Nifti1Image(icc_map, affine), output_dir / f"{output_prefix}_icc.nii.gz")
    
    logger.info(f"Saved: {output_prefix}_zmap.nii.gz, {output_prefix}_pmap.nii.gz, "
                f"{output_prefix}_beta.nii.gz, {output_prefix}_icc.nii.gz")
    ...
    nib.save(nib.Nifti1Image(fdr_map, affine), output_dir / f"{output_prefix}_fdr05.nii.gz")
    
    logger.info(f"Saved: {output_prefix}_fdr05.nii.gz")
...
def main():
    ...
    parser.add_argument("--no-smooth", action="store_true",
                       help="Skip smoothing (for sensitivity analysis). "
                            "Default: apply SMOOTH_FWHM to match GLM pipeline.")
    parser.add_argument("--no-run-effect", action="store_true",
                       help="Fit intercept-only LME (beta ~ 1 + (1|subject)), "
                            "omitting the run fixed effect. This is the "
                            "intermediate model for Major Concern 2, which "
                            "isolates the contribution of hierarchical modeling "
                            "from the run covariate.")
    args = parser.parse_args()
    
    LME_VOXELWISE.mkdir(parents=True, exist_ok=True)
    
    smooth = None if args.no_smooth else SMOOTH_FWHM
    if smooth:
        logger.info(f"Smoothing enabled: {smooth} mm FWHM (matches GLM pipeline)")
    else:
        logger.info("Smoothing DISABLED (--no-smooth flag). "
                     "NOTE: GLM uses smoothing_fwhm={} in SecondLevelModel.".format(SMOOTH_FWHM))

    subject_ids = get_subject_ids()
    beta_imgs, run_info = load_all_runs_betas(subject_ids, smooth_fwhm=smooth)
    ...
    include_run = not args.no_run_effect
    if not include_run:
        logger.info("="*60)
        logger.info("INTERMEDIATE MODEL (Major Concern 2): beta ~ 1 + (1|subject)")
        logger.info("Run fixed effect EXCLUDED for fair comparison with GLM t-test")
        logger.info("="*60)
        output_prefix = "lme_norun"
    else:
        output_prefix = "lme"
    ...
    checkpoint_path = LME_VOXELWISE / f"checkpoint_{output_prefix}.pkl"
    ...
    results_df = run_lme_voxelwise_chunked(
        ...
        resume=args.resume,
        include_run_effect=include_run
    )
    
    results_df.to_csv(LME_VOXELWISE / f"{output_prefix}_results.csv", index=False)
