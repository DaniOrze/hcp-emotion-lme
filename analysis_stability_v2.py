...
def _fit_lme_safe(df, formula=LME_FORMULA, method="powell", maxiter=100):
    try:
        ...
        run_info = df[['subject', 'run']].copy() if 'run' in df.columns else df[['subject']].copy()
        
        include_run = ('run' in formula and 'run' in df.columns and df['run'].nunique() >= 2)
        ...
        if res.get('converged', False):
            return res['fixed_effect'], res.get('t_value', res.get('z_score', np.nan))
        else:
...
def bootstrap_lme(df, n_bootstrap=1000, random_state=42):
    ...
    for _ in range(n_bootstrap):
        ...
        try:
            ...
            include_run = boot_df['run'].nunique() >= 2
            ...
            if res.get('converged', False):
                boot_intercepts.append(res['fixed_effect'])
                boot_t.append(res.get('t_value', res.get('z_score', np.nan)))
        except Exception:
            continue
